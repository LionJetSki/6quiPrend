...
class Main {
  ...
  private static void permuter(int j, int jmin,ArrayList<Integer>tab_boeuf,ArrayList<String>tab_noms) {
      int stockage = tab_boeuf.get(j);
      String stockage_str = tab_noms.get(j);
      
      int stockageplus = tab_boeuf.get(jmin);
      String stockage_stringplus = tab_noms.get(jmin);

      tab_boeuf.remove(jmin);
      tab_noms.remove(jmin);
      
      tab_boeuf.add(jmin,stockage);
      tab_noms.add(jmin,stockage_str);
      
      
      tab_boeuf.remove(j);
      tab_noms.remove(j);
      
      tab_boeuf.add(j,stockageplus);
      tab_noms.add(j,stockage_stringplus);
      
    }
  
  public static void tri_fin_de_manche(ArrayList<Integer> tab_boeuf,ArrayList<String>tab_noms ) {

      for (int i = 0; i <tab_noms.size(); ++i) {
        int jmin = i;
        for (int  j = i+1; j <tab_noms.size(); ++j) {
          if(tab_noms.get(j).compareTo(tab_noms.get(jmin)) < 0 ){
              permuter(j,jmin,tab_boeuf,tab_noms);
          }
        }
      } 
  }
  ...
  public static void bilan_fin_de_manche(ArrayList<Integer> tetes_boeuf_ramasser_tmp,ArrayList<String>noms_tmp ){
    ...
    String tmp = "";
      for (int i = 0; i < tetes_boeuf_ramasser_tmp.size() ; ++i ){
        if(tetes_boeuf_ramasser_tmp.get(i) > 1){
          tmp="s";
        }
        if(tetes_boeuf_ramasser_tmp.get(i) > 0){
           System.out.println(noms_tmp.get(i) +" a ramassé " + tetes_boeuf_ramasser_tmp.get(i) + " tête"+ tmp+ " de boeufs");
        }
        else{
          ++compteur;
        }
  ...
  public static boolean poser_carte_trop_petite(Partie partie,
    ...
    if(partie.carte_trop_petite(carte_choisies_tmp.get(0))){
      ...
      tri_fin_de_manche(tetes_boeuf_ramasser_tmp,noms_tmp );
   ...
   public static void affichage_final(Partie partie, 
     ...
     String tmp = "";
      ...
      for(int k = 0;k < partie.getNb_joueurs();++k){
        if(tetes_boeuf_ramasser_tmp.get(k) > 1){
          tmp="s";
        }
        System.out.println( noms_tmp.get(k)+" a ramassé " + tetes_boeuf_ramasser_tmp.get(k)+ " tête"+ tmp+ " de boeufs");
      }
   }
